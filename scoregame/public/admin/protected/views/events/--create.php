...
if ($idevent=$_GET['idevent']) {
	...
	if (!$event) $this->redirect('index.php?r=events/create');
...
<h1> <? echo ( Yii::app()->user->getState('battingteam') == Yii::app()->user->getState('idvisitingteam') ) ? 'TOP ' : 'BOTTOM' ?> OF THE <?php echo $model->Inning ?>
	<?  switch($model->Inning){
		case 1: 
			echo 'ST';
		break;
